...
namespace Main
{
    ...
    public partial class Main : Form
    {
        ...
        {
            string configFilePath = "coeff.txt";
            try
            {
                if (File.Exists(configFilePath))
                {
                    ...
                    if (lines.Length > 0)
                    {
                        if (double.TryParse(lines[0], out double savedValue))
                        {
                            CorrectionCoeff = savedValue;
                ...
                {
                    // Файл не существует - информационное сообщение
                    MessageBox.Show(
                        "Файл коррекции коэффициента не найден. Будут использованы значения по умолчанию (1).",
                        "Информация",
        ...
        private void SaveDataToFile()
        {
            ...
            try
            {
                File.WriteAllText(configFilePath, CorrectionCoeff.ToString());
